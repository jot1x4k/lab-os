...
bool estrategia_desde_texto(const std::string &texto, Estrategia &destino)
{
    ...
    if (t == "fifo")
    {
        destino = Estrategia::FIFO;
        return true;
    }
    if (t == "sjf")
    {
        destino = Estrategia::SJF;
        return true;
    }
    if (t == "rr")
    {
        destino = Estrategia::RR;
        return true;
    }
    if (t == "srt")
    {
        destino = Estrategia::SRT;
        return true;
    }
    return false;
}
...
Resultado planificar(std::vector<Proceso> &procesos, std::vector<Cola_prioridad> &colas, bool traza)
{
    ...
    while (por_terminar > 0)
    {
        ...
        {
            for (auto it = cola.llegada.begin(); it != cola.llegada.end(); ++it)
            {
                Proceso *candidato = *it;
                if (candidato->llegada >= ahora + quantum_asignado)
                {
                    break;
                }
                int restante_actual_en_llegada = actual->restante - (candidato->llegada - ahora);
                if (candidato->restante < restante_actual_en_llegada)
                {
                    quantum_asignado = candidato->llegada - ahora;
